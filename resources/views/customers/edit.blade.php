                    ...
                    <form method="post" action="{{ route('customers.update', $customer->id) }}" class="mt-6 space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="first_name" name="first_name" type="text" class="mt-1 block w-full"
                                required autofocus autocomplete="first_name" :value="old('first_name', $customer->first_name)"/>
                        ...
                        <div>
                            ...
                            <x-text-input id="last_name" name="last_name" type="text" class="mt-1 block w-full"
                                required autofocus autocomplete="last_name" :value="old('last_name', $customer->last_name)"/>
                        ...
                        <div>
                            ...
                            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full"
                                required autocomplete="email" aria-placeholder="Email" :value="old('email', $customer->email)"/>
                        ...
                        <div>
                            ...
                            <select id="group_id" name="group_id[]" type="group_id" class="mt-1 block w-full" required multiple>
                                <option {{empty($customerGroups) ? 'selected' : ''}} value="">{{ __('Select Group') }}</option>
                                @foreach ($groups as $key=>$value)
                                    <option value="{{ $value }}" {{ in_array($key, $customerGroups) ? 'selected' : '' }}>
                                        {{ $key }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="gender" name="gender" type="gender" class="mt-1 block w-full">
                                <option {{empty($customerGroups) ? 'selected' : ''}} value="">{{ __('Select Gender') }}</option>
                                @foreach ($genderTypes as $type)
                                    <option value="{{ $type->value }}" {{ ($customer->gender && $customer->gender->value == $type->value) ? 'selected' : '' }}>
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-flatpickr date-format="Y-m-d" :max-date="today()" for="birth_date" name="birth_date" :value="old('birth_date', $customer->birth_date)"/>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            @if (session('status') === 'customer-not-updated')
                                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
